...
def add_obstacles_corner():
    ...
    gray = (0.6, 0.6, 0.6)
    red = (0.9, 0.1, 0.1)
    # 차선폭 2.8m: wedge(2.4m)와 완주-매끈(3.0m) 사이. 완주하며 약한 후진 기대.
    bottom = _wall(0.0, -1.4, 3.8, -1.4)    # 진입차선 바닥벽
    outer = _wall(3.8, -1.4, 3.8, 5.0)      # 정면벽 + 출구차선 바깥벽
    topentry = _wall(0.0, 1.4, 1.0, 1.4)    # 진입차선 윗벽 (짧게)
    inner = _wall(1.0, 1.4, 1.0, 5.0)       # 출구차선 안쪽벽 (폭 2.8m)
    ...
    print(f"[corner] 벽 큐브 {counter[0]}개. 정면벽(x=3.8) 만나 좌 90° 꺾어 진입(폭 2.8m).")
    print("  목표 (2.4,4.0). send_goal.sh 2.4 4.0")
